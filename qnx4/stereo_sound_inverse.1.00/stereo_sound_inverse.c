...
/*
 $Log: stereo_sound_inverse.c,v $
 Revision 1.0  2023/09/04 egthomas
 Initial revision from stereo_sound
...
char cmdlne[1024];
char progid[80]={"stereo_sound_inverse.c 1.0 2023/09/04 egthomas"};
char progname[256];
...
int main(int argc,char *argv[]) {
  ...
  /* Channel A sounding frequencies */
  if (stid == 41) {
    ifreqsA[0] = 1;
    ifreqsA[1] = 3;
    ifreqsA[2] = 4;
...
printf("***ifreqsB_index=%d\n",ifreqsB_index);
      ...
      /* Channel A uses sounding frequencies */
      stfrqA=usfreq[ifreqsAband];
      frqrngA=ufreq_range[ifreqsAband];

      /* Channel B uses fixed frequencies */
      stfrqB=usfreq[dfrqB];
